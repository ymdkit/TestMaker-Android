...
class EditSelectCompleteQuestionFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        editSelectCompleteQuestionViewModel.imagePath.observeNonNull(viewLifecycleOwner) {
        ...
        return DataBindingUtil.inflate<FragmentEditSelectCompleteQuestionBinding>(inflater, R.layout.fragment_edit_select_complete_question, container, false).apply {
            ...
            buttonImage.setOnClickListener {
                AlertDialog.Builder(requireContext(), R.style.MyAlertDialogStyle)
                        .setItems(
                                if (!editSelectCompleteQuestionViewModel.imagePath.value.isNullOrEmpty()) resources.getStringArray(R.array.action_image) else resources.getStringArray(R.array.action_image).take(2).toTypedArray()
                        ) { _, which ->
                            ...
                            when (which) {
                                CAMERA -> {
                                    if (ContextCompat.checkSelfPermission(requireContext(), Manifest.permission.CAMERA)
                                        ...
                                        requestPermissions(
                                                ...
                                                EditWriteQuestionFragment.REQUEST_PERMISSION_CAMERA)
                                    } else {
                                        takePicture()
                                    }
                                }
                                GALLERY -> {
                                    ...
                                    intent.type = "image/*"
                                    startActivityForResult(intent, EditWriteQuestionFragment.REQUEST_SAF_PICK_IMAGE)
                                }
                                REMOVE -> {
                                    editSelectCompleteQuestionViewModel.imagePath.value = ""
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        val bitmap = when (requestCode) {
            EditWriteQuestionFragment.REQUEST_SAF_PICK_IMAGE -> getBitmapFromUri(data.data)
        ...
        try {
            ...
            AlertDialog.Builder(requireContext(), R.style.MyAlertDialogStyle)
                    ...
                    .setPositiveButton(android.R.string.ok) { _, _ ->
                        ...
                        editSelectCompleteQuestionViewModel.imagePath.value = fileName
                        binding?.buttonImage?.setImageWithGlide(requireContext(), cropView.croppedBitmap)

                        lifecycleScope.launch {
                            editSelectCompleteQuestionViewModel.imagePath.value?.let {
