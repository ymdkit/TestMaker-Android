...
class EditSelectQuestionFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        editSelectQuestionViewModel.imagePath.observeNonNull(viewLifecycleOwner) {
        ...
        return DataBindingUtil.inflate<FragmentEditSelectQuestionBinding>(inflater, R.layout.fragment_edit_select_question, container, false).apply {
            ...
            buttonImage.setOnClickListener {
                AlertDialog.Builder(requireContext(), R.style.MyAlertDialogStyle)
                        .setItems(
                                if (!editSelectQuestionViewModel.imagePath.value.isNullOrEmpty()) resources.getStringArray(R.array.action_image) else resources.getStringArray(R.array.action_image).take(2).toTypedArray()
                        ) { _, which ->
                            ...
                            when (which) {
                                ...
                                REMOVE -> {
                                    editSelectQuestionViewModel.imagePath.value = ""
        ...
        try {
            ...
            AlertDialog.Builder(requireContext(), R.style.MyAlertDialogStyle)
                    ...
                    .setPositiveButton(android.R.string.ok) { _, _ ->
                        ...
                        editSelectQuestionViewModel.imagePath.value = fileName
                        binding?.buttonImage?.setImageWithGlide(requireContext(), cropView.croppedBitmap)

                        lifecycleScope.launch {
                            editSelectQuestionViewModel.imagePath.value?.let {
