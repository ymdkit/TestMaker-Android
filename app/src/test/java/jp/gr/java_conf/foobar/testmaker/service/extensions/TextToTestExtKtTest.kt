...
class TextToTestExtKtTest {
    ...
    fun toTest() {
        ...
        assertEquals("記述問題の形式", Constants.WRITE, test.questionsNonNull()[0]?.type ?: -1)
        assertEquals("記述問題の解答（複数）", 0, test.questionsNonNull()[0]?.answers?.size ?: -1)
        assertEquals("記述問題の選択肢（複数）", 0, test.questionsNonNull()[0]?.selections?.size ?: -1)
        assertEquals("記述問題の解説", "解説文", test.questionsNonNull()[0]?.explanation ?: "a")
        ...
        assertEquals("完答問題の形式", Constants.COMPLETE, test.questionsNonNull()[0]?.type ?: -1)
        assertEquals("完答問題の解答（複数）", 4, test.questionsNonNull()[0]?.answers?.size ?: -1)
        test.questionsNonNull()[0]?.answers?.forEachIndexed { index, select ->
            assertEquals("完答問題の解答", "答え${index + 1}", select.selection ?: "")
        }
        assertEquals("完答問題の選択肢（複数）", 0, test.questionsNonNull()[0]?.selections?.size ?: -1)
        assertEquals("完答問題の解説", "", test.questionsNonNull()[0]?.explanation ?: "a")
        assertEquals("完答問題の解答順", false, test.questionsNonNull()[0]?.isCheckOrder ?: "a")


        line = "${context.getString(R.string.load_multiple_answers_order)},完答問題の問題文,答え1,答え2,答え3,答え4"
        test = line.toTest(context)
        assertEquals("完答問題の解答順", true, test.questionsNonNull()[0]?.isCheckOrder ?: "a")
        ...
        assertEquals("選択問題の形式", Constants.SELECT, test.questionsNonNull()[0]?.type ?: -1)
        assertEquals("選択問題の解答（複数）", 0, test.questionsNonNull()[0]?.answers?.size ?: -1)
        assertEquals("選択問題の選択肢（複数）", 4, test.questionsNonNull()[0]?.selections?.size ?: -1)
        test.questionsNonNull()[0]?.selections?.forEachIndexed { index, select ->
            assertEquals("選択問題の選択肢", "外れ${index + 1}", select.selection ?: "")
        }
        assertEquals("選択問題の解説", "", test.questionsNonNull()[0]?.explanation ?: "a")
        assertEquals("選択問題の自動生成", false, test.questionsNonNull()[0]?.auto ?: "a")

        line = "${context.getString(R.string.load_selection_auto_problems)},選択問題の問題文,選択問題の答え,5"
        test = line.toTest(context)

        assertEquals("選択問題の選択肢（複数）", 5, test.questionsNonNull()[0]?.selections?.size ?: -1)
        assertEquals("選択問題の自動生成", true, test.questionsNonNull()[0]?.auto ?: "a")
        ...
        assertEquals("選択完答問題の形式", Constants.SELECT_COMPLETE, test.questionsNonNull()[0]?.type ?: -1)
        assertEquals("選択完答問題の解答（複数）", 2, test.questionsNonNull()[0]?.answers?.size ?: -1)
        test.questionsNonNull()[0]?.answers?.forEachIndexed { index, select ->
            assertEquals("選択完答問題の解答", "答え${index + 1}", select.selection ?: "")
        }
        assertEquals("選択完答問題の選択肢（複数）", 4, test.questionsNonNull()[0]?.selections?.size ?: -1)
        test.questionsNonNull()[0]?.selections?.forEachIndexed { index, select ->
            assertEquals("選択完答問題の選択肢", "外れ${index + 1}", select.selection ?: "")
        }
        assertEquals("選択完答問題の解説", "", test.questionsNonNull()[0]?.explanation ?: "a")
        assertEquals("選択問題の自動生成", false, test.questionsNonNull()[0]?.auto ?: "a")
        ...
        assertEquals("選択問題の自動生成", true, test.questionsNonNull()[0]?.auto ?: "a")
        assertEquals("選択完答問題の解答（複数）", 2, test.questionsNonNull()[0]?.answers?.size ?: -1)
        assertEquals("選択完答問題の選択肢（複数）", 2, test.questionsNonNull()[0]?.selections?.size ?: -1)
        ...
        assertEquals("問題集のタイトル", "タイトル", test.title ?: "a")
